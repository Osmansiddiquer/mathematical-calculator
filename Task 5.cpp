...
int main()
{
	...
	printf("\n/////////  Highly Developed Calculator  \\\\\\\\\\\\\\\\\\\\");
	printf("\n\tOperates using 8 arithmetic operators.\n");
	...
	//Some Syntax Error checking before proceeding and calculating length
	...
	for(int i=length-1; i>=0; i--)  //evaluating binary '^' and 'r with right associativity
	{
		if(expression[i] == '^' || expression[i] == 'r'){
			op1 = 0;
			op2 = 0;
			dec_position = -1;
			int Sign;
			...
			else
			{
				...
				{
					printf("\n______________________________________________\n\n");
					printf("MATH ERROR: 0TH root calculated at position %d\n", i);
					printf("______________________________________________\n");
					goto end;
				}
				if(int(index)%2==0&&op2<0)
				{
					printf("\n______________________________________________\n\n");
					printf("     MATH ERROR: Undefined taking even root \n\tof negative number at position %d\n", i+2);
					printf("______________________________________________\n");
					goto end;
				}
				if(op2==0)
				...
				{
					result = 1;
					double result_to_power_n = 1;
					while(result_to_power_n>op2+1e-5 || result_to_power_n<op2-1e-5){
						result = result - (result_to_power_n-op2)/(index*result_to_power_n/result);  //using newton's method to solve for result^n-op1=0
			    		result_to_power_n=1;
						for(int i=0; i<index; i++)
							result_to_power_n *= result;
					}
				}	
			}
			buffer = expression;
